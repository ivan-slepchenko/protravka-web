...
const initialState: CropsState = { crops: [] };
...
export const createVariety = createAsyncThunk(
    'crops/createVariety',
    async ({ cropId, variety }: { cropId: string; variety: Variety }) => {
...
export const deleteVariety = createAsyncThunk(
    'crops/deleteVariety',
    async ({ cropId, varietyId }: { cropId: string; varietyId: string }) => {
...
const cropsSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(
            deleteVariety.fulfilled,
            (state, action: PayloadAction<{ cropId: string; varietyId: string }>) => {
                const crop = state.crops.find((crop) => crop.id === action.payload.cropId);
                if (crop) {
                    crop.varieties = crop.varieties.filter(
                        (variety) => variety.id !== action.payload.varietyId,
                    );
                }
            },
        );
